...
## Manage the databsae
```bash
docker exec -it productivitytoolsphotoprism-mariadb-1 bash
apt-get update
...
```sql
select album_path, album_category, album_title,convert(album_order, char(200)), convert(album_type, char(200)), album_type from albums where convert(album_type, char(100))='folder';
...
- Copy files to docker where maria db is deployed

File from the PhotoPrism git repository
```
docker cp UpdateDB.sh 4200471e7c42:\UpdateDB.sh
```
File from the Home.Configuration repository
``` 
docker cp Prism.env 4200471e7c42:\Prism.env
```
File looks like this:
```
ENV_PHOTOPRISM_ADMIN_PASSWORD=pass
ENV_PHOTOPRISM_DATABASE_PASSWORD=pass
```

- On the docker instance set the env variable (Not needed)
```
...
## Denug docker
```
docker logs --tail 200 productivitytoolsphotoprism-photoprism-1
docker inspect productivitytoolsphotoprism-photoprism-1 --format '{{.State.OOMKilled}}'
```



----OLD: sets categories to old albums, for debbug purpose,
you can invoke that and old categories should show, and later the cron should clean them
